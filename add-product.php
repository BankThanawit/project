<?php
	...
	if(empty($product_name) || empty($product_price) || empty($product_stype) || empty($product_image)){
        ...
        $insert = "INSERT INTO product(product_name, product_price, product_stype, imags) VALUES('$product_name', '$product_price', '$product_image', '$product_image')";
        $upload = mysqli_query($conn,$insert);
        if($upload){
           move_uploaded_file($product_image_tmp_name, $product_image_folder);
           echo '<script>alert("new product added successfully");
                window.location.href = "http://localhost/mini/edit-pro.html"
                </script>';
        }else{
