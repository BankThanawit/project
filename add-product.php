...
$imags = $_FILES['imags']['name'];
$product_image_tmp_name = $_FILES['imags']['tmp_name'];
$product_image_folder = 'imags/' . $imags;

$conn = new mysqli('localhost', 'root', '', 'mnp');
if (empty($product_name) || empty($product_price) || empty($product_stype) || empty($imags)) {
   $message[] = 'please fill out all';
} else {
   $insert = "INSERT INTO product(product_name, product_price, product_stype, imags) VALUES('$product_name', '$product_price', '$product_stype', '$imags')";
